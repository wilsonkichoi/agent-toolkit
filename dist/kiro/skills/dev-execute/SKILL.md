---
name: dev-execute
description: "This skill should be used when the user asks to \"execute the next task\", \"pick up a task\", \"work on task <id>\", \"start the execution loop\", \"implement the next ticket\", or invokes /dev-execute. Claims one task from the tracker, implements it in an isolated worktree, opens a PR, drives CI to green, and hands off at In Review. Never merges."
...
> **Generated Kiro preview.** Invoke this skill as `/dev-execute` with trailing context matching `[task-id | external #issue]`. Resolve bundled
...
run one, render the Kiro invocation as `/dev-verify`.

Read first: `.agent-toolkit/dev.md` (tracker routing config), the plugin's
`references/runtime_contracts/tracker.md` (tracker
verbs, backend mapping, next-task algorithm), and the plugin's
`references/runtime_contracts/project-bootstrap.md` (execution-repository instructions and deterministic rule loading).
On Claude Code these plugin docs are under `references/runtime_contracts/`; equivalently they are
under `references/runtime_contracts/` relative to this skill's directory.
...
- `claim` the task and confirm the claim won (re-read; see `references/runtime_contracts/tracker.md` race guard). For a
...
After fetching and validating the packet, follow `references/runtime_contracts/project-bootstrap.md` before the first
...
this item (`references/runtime_contracts/tracker.md` "Secondary intake channel"). Same worktree → PR → CI → review → verify
...
Before implementation, rerun `references/runtime_contracts/project-bootstrap.md` in the new task worktree with its branch
...
2. Post the work-summary comment on the task:
   ...
   plugin before invoking it. In Kiro the script is
