...
Read first: `.claude/dev.md`, the plugin's `references/tracker.md` (on Claude Code
`references/tracker.md`, equivalently `references/tracker.md` relative to this
...
3. **Create at `Backlog`** via `create-task`, wiring dependencies as native relations per
   the backend section of `references/tracker.md` (both directions from
...
(`references/tracker.md` "Secondary intake channel"). `gh issue view <n>` for the request, then run the same triage
...
the remaining `status:*` label - closed issues carry none, see `references/tracker.md`). The reason
