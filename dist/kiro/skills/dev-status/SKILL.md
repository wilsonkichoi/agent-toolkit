---
name: dev-status
description: "This skill should be used when the user asks \"where are we\", \"project status\", \"what's next to work on\", \"show the board\", \"dev status\", or invokes /dev-status. One read-only screen: milestone progress from the tracker, open PRs with CI state, WIP against the limit, blocked tasks, and the next claimable tasks. Changes nothing."
...
> **Generated Kiro preview.** Invoke this skill as `/dev-status` with trailing context matching `[milestone N]`. Resolve bundled
...
Read first: `.agent-toolkit/dev.md` and the plugin's `references/runtime_contracts/tracker.md` — on Claude Code
`references/runtime_contracts/tracker.md`, equivalently `references/runtime_contracts/tracker.md` relative to this
