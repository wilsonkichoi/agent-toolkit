...
Read first: `.claude/dev.md` and the plugin's `references/tracker.md` — on Claude Code
...
2. **PRs:** open PRs on `task/*` branches (`gh pr list`), each with CI state
   ...
   `secondary_intake: github` is set (`references/tracker.md`), a `task/*` PR that links a `#N` issue but
