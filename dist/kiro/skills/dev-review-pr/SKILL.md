---
name: dev-review-pr
description: "This skill should be used when the user asks to \"review the PR\", \"review task <id>'s PR\", \"run the dev review\", \"apply review fixes\", \"address review comments\", or invokes /dev-review-pr. Reviews a task's pull request against its packet (DoD, spec excerpts) and posts one structured verdict; in fix mode, applies one current findings batch and pushes, then stops. Never merges."
compatibility: Kiro IDE single-root workspace preview; CLI, multi-root, explicit resources, and dev:shadow unsupported
metadata:
  source-plugin: dev
  source-skill: "review-pr"
  generated: "true"
---

> **Generated Kiro preview.** Invoke this skill as `/dev-review-pr` with trailing context matching `[pr-number | task-id] [fix]`. Resolve bundled
...
`references/runtime_contracts/project-bootstrap.md` before gathering the diff, spec, CI, or review evidence. Pass every
...
in-place `#N` PR, or a drive-by PR with no issue at all - `references/runtime_contracts/tracker.md` repository
