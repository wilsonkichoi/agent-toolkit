...
1. **Load tracker routing only.** Read the tracker repository's dev configuration at
   ...
   `references/runtime_contracts/tracker.md`. Resolve the tracker and GitHub repository roles once. Do not treat the
...
3. **Build the rule context.** Run the resolver from the installed plugin, passing the exact
   ...
   the script is under `scripts/`; on Codex it is
...
```text
uv run scripts/resolve_project_rules.py \
  --tracker-repo <path> \
  --execution-repo <path> \
...
```text
uv run scripts/resolve_project_rules.py --check <repo>
```
