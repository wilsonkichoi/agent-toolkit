---
name: dev-release
description: "Publish the GitHub Release for an existing plugin version tag. Use when the user says \"publish the release\", \"create the GitHub release for dev-v0.0.69\", \"release the plugin version\", \"cut the release for this tag\", or invokes dev:release. The tag itself is created automatically when a version bump merges to main; this skill only publishes its release. For merging a pull request use dev:merge-pr, and for task verification use dev:verify."
...
> **Generated Kiro preview.** Invoke this skill as `/dev-release` with trailing context matching `<plugin>-vX.Y.Z`. Resolve bundled
...
On Claude Code the helper is `scripts/plugin_release.py`. On Codex it is
`scripts/plugin_release.py` relative to this `SKILL.md`. Invoke it exactly once, after
authorization:

```bash
uv run scripts/plugin_release.py release \
