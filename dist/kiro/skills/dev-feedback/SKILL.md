---
name: dev-feedback
description: "This skill should be used when the user says \"file a bug against agent-toolkit\", \"report this issue to the plugin\", \"feedback for agent-toolkit\", \"this is a dev plugin bug\", \"submit an enhancement request\", or invokes /dev-feedback. Turns problems and improvement ideas discovered during dev lifecycle runs into structured, deduplicated GitHub issues in the agent-toolkit repository."
...
> **Generated Kiro preview.** Invoke this skill as `/dev-feedback` with trailing context matching `[bug | enhancement | docs | workflow] <description>`. Resolve bundled
...
`scripts/feedback_redact.py`; on Codex it is
...
```bash
uv run scripts/feedback_redact.py redact < /tmp/field.txt
```

Or pipe via heredoc when the text is known safe (no unmatched quotes):

```bash
uv run scripts/feedback_redact.py redact <<'REDACT_EOF'
<text to redact>
REDACT_EOF
```
...
- Single-slash relative paths with recognized extensions (`references/runtime_contracts/project-bootstrap.md`,
  `scripts/tool.py`, `config/settings.yml`, `src/module.mjs`)
- Multi-segment relative paths (three or more segments, e.g. `plugins/dev/skills/dev-feedback/SKILL.md`)
...
```bash
uv run scripts/feedback_redact.py access
```
...
```bash
uv run scripts/feedback_redact.py search 'keyword1 keyword2' 'symptom phrase'
```
...
```bash
uv run scripts/feedback_redact.py draft --input /tmp/feedback-draft.json
```
