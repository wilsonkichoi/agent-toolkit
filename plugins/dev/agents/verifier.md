...
1. Gather your own inputs; the only caller-relayed content you accept is the PR number, the
   ...
   malformed; report a failed execute handoff instead of dropping the `In Review` precondition.
   `external` and `secondary` have no queue state. Never create or repair `In Progress`, `In
