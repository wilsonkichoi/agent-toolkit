...
plugin before invocation. On Claude Code `<plugin-root>` is `${CLAUDE_PLUGIN_ROOT}`; on Codex the script is
`../../scripts/work_summary.py` relative to the invoking dev skill's `SKILL.md`. The Claude Code
validator is `${CLAUDE_PLUGIN_ROOT}/scripts/work_summary.py`. Resolve the
Codex-relative form against that skill directory, not the adopter repository or process cwd, and
