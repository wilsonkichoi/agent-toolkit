...
[![Dev plugin lifecycle diagram](diagram/dev-plugin-diagram.png)](https://wilsonkichoi.github.io/dev-plugin-diagram.html)
...
| `/dev:setup` | Initialize a project (greenfield or brownfield): scaffold `docs/`, pick tracker backend, write `.agent-toolkit/dev.md`. Brownfield mode offers architecture archaeology into a current-state SPEC.md. Migrates a pre-0.0.64 rule registry to `tier` frontmatter. Optional installers for the auto-review GitHub Action and the `check-rules` CI step. |
...
| `/dev:execute` | Claim one task → git worktree → implement → tests (via the `test-writer` agent, contract-only context) → PR → CI to green → visual self-check + local preview instructions (when DoD has visual criteria; the executor inspects touched pages against the comparison target before hand-off) → validated work-summary comment → `In Review`. Never merges. Safeguards: `work_in_progress_limit`, `max_fix_attempts`, packet validation for hand-written tickets, shared work-summary validation, and verified write-then-read lifecycle transitions for planned GitHub tasks. |
