...
| `/dev:execute` | Claim one task → git worktree → implement → tests (via the `test-writer` agent, contract-only context) → PR → CI to green → work-summary comment → `In Review`. Never merges. Safeguards: `work_in_progress_limit`, `max_fix_attempts`, packet validation for hand-written tickets. |
| `/dev:auto` | Unattended per-task pipeline: execute → independent review → bounded fix loop → verify → merge → record-only retro, then next task, so dependency chains progress to `Done`. Requires `auto_merge: true` (standing approval); merges only mechanically-evidenced, review-approved work; manual DoD criteria always stop for a human. |
