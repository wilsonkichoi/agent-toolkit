...
| `/dev:shadow` | Unattended historical-replay evaluation (`/dev:shadow #<issue> [pr <n>]`): reconstruct a completed issue's historical base, re-implement it with the active session's model through execute → review → bounded fix → verify, then compare against the original on tests, DoD coverage, review findings, scope, time, tokens, and estimated API-equivalent cost. Posts an audit report on an isolated `[SHADOW]` issue. Never merges; never mutates the source issue or original PR. GitHub source only in v0. |
