...
| `dev:auto` | 6-8 | Unattended per-task pipeline for an exact task id or a milestone: execute → independent review → bounded fix loop → verify → merge (only under `auto_merge` conditions) → record-only retro. It alone chains separate review and fix invocations, bounded by `max_fix_attempts`. Task-id mode is strictly one task with no fallthrough; milestone mode advances sequentially. Single-flight; stops on Blocked, unmet/manual DoD criteria, merge conflicts, or `max_tasks_per_run`. |
| `dev:review-pr` | 6 | Fresh-session reviewer. Fetches PR diff + CI results + linked task packet; checks DoD compliance, spec compliance, code quality; posts one structured PR review via `gh` with verdict (approve / request changes), then stops. A manual fix invocation snapshots one current findings batch, applies fixes, tests, pushes, replies, requests or records the need for re-review, then stops without executing another review or fix pass. |
